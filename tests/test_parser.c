...
void	test_parser_only_1_each_unique_element(void)
{
	...
	TEST_ASSERT_EQUAL_DOUBLE(255.0 / 255.0, scene->ambience->rgb->red);
	TEST_ASSERT_EQUAL_DOUBLE(250.0 / 255.0, scene->ambience->rgb->green);
	TEST_ASSERT_EQUAL_DOUBLE(245.0 / 255.0, scene->ambience->rgb->blue);
	...
	TEST_ASSERT_EQUAL_DOUBLE(250.0 / 255.0, scene->light->rgb->red);
	TEST_ASSERT_EQUAL_DOUBLE(255.0 / 255.0, scene->light->rgb->green);
	TEST_ASSERT_EQUAL_DOUBLE(245.0 / 255.0, scene->light->rgb->blue);
	aux = scene->objects;
	while (aux)
	{
		...
		if (obj->type == ID_CYLINDER)
		{
			...
			TEST_ASSERT_EQUAL_DOUBLE(90.0 / 255.0, obj->rgb->green);
			TEST_ASSERT_EQUAL_DOUBLE(201.0 / 255.0, obj->rgb->blue);
		...
		else if (obj->type == ID_SPHERE)
		{
			...
			TEST_ASSERT_EQUAL_DOUBLE(10.0 / 255.0, obj->rgb->red);
			TEST_ASSERT_EQUAL_DOUBLE(0.0 / 255.0, obj->rgb->green);
			TEST_ASSERT_EQUAL_DOUBLE(255.0 / 255.0, obj->rgb->blue);

...
/*   Updated: 2022/11/01 21:13:14 by maolivei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_shape_color(char *token, t_object *shape)
{
	...
	if (!rgb || ft_splitsize(rgb) != 3)
	{
		...
		return (ft_putendl_fd(ERR_SHP_COLOR_SETTN, STDERR), -1);
	}
	if (!ft_isnumber(rgb[0]) || !ft_isnumber(rgb[1]) || !ft_isnumber(rgb[2]))
	{
		ft_free_matrix((void *)&rgb);
		return (ft_putendl_fd(ERR_SHP_COLOR_VALUE, STDERR), -1);
	}
	aux[0] = ft_atoi(rgb[0]);
	aux[1] = ft_atoi(rgb[1]);
	aux[2] = ft_atoi(rgb[2]);
	...
	|| !ft_isinrange(aux[2], 0, 255))
		return (ft_putendl_fd(ERR_SHP_COLOR_RANGE, STDERR), -1);
...
int	set_shape_orientation_vector(char *token, t_object *shape)
{
	...
	{
		ft_free_matrix((void *)&vec);
		return (ft_putendl_fd(ERR_SHP_OVECT_VALUE, STDERR), -1);
	}
	aux[0] = ft_atof(vec[0]);
	aux[1] = ft_atof(vec[1]);
	aux[2] = ft_atof(vec[2]);
	...
	|| !ft_isinrange(aux[2], -1, 1))
		return (ft_putendl_fd(ERR_SHP_OVECT_RANGE, STDERR), -1);
...
int	set_shape_material(t_object *shape, t_rt_scene *s)
{
	if (!s->ambient || !s->light)
		return (ft_putendl_fd(ERR_SHP_MISS_INFO, STDERR), -1);
